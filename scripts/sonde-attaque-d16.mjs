...
// ══════════ CE QUE L'ATTAQUE AJOUTE — rien n'est retire ═════════════════════
// `--cpu N`   bride le fil principal par CDP (Emulation.setCPUThrottlingRate).
// `--lieu "lat,lon"` + `--zoom-lieu z` : on va d'abord LA, puis on remonte en
//             orbite depuis ce lieu. Sans ca toute mesure vaut « pleine mer ».
// `--grille LxH` : la finesse du condense d'image (16x10 par defaut). Une
//             rupture qui occupe 2 % de l'ecran est invisible a 16x10.
// scenarios ajoutes : `clic-suite`, `pano-descente`, `resize-descente`, `z16`.
const CPU = Number(opt('--cpu', '1'))
const LIEU = opt('--lieu', '')
const ZOOM_LIEU = Number(opt('--zoom-lieu', '8'))
const GRILLE = opt('--grille', '16x10')
const [G_L, G_H] = GRILLE.split('x').map(Number)
...
function poserInstrument(GL_, GH_) {
...
try {
  ...
  console.log('instrument :', await page.evaluate(poserInstrument, G_L, G_H))
  ...
  if (IMAGES || SCENARIO === 'z16') fs.mkdirSync(dossierImg, { recursive: true })

  // ── LE LIEU : on y va d'abord, puis on remonte en orbite depuis LÀ ────────
  // ⚠️ Sans ça, toute mesure de ce chantier vaut « La Réunion, pleine mer ».
  if (LIEU) {
    const [la, lo] = LIEU.split(',').map((x) => Number(x.trim()))
    sortie.lieu = { lat: la, lon: lo, zoom: ZOOM_LIEU }
    sortie.lieuOk = await page.evaluate(async (a, b, z) => {
      try { return !!(await window.__exp.modes.flyTo(a, b, z)) } catch (er) { return 'err:' + er.message }
    }, la, lo, ZOOM_LIEU)
    await dodo(9000)
    sortie.lieuArrive = await page.evaluate(() => ({
      lat: window.__exp.dem?.lat, lon: window.__exp.dem?.lon, zoom: window.__exp.dem?.zoom,
      alt: window.__exp.altitudeCadrageM?.(), mode: window.__exp.modes?.mode,
    }))
    console.log('lieu :', JSON.stringify(sortie.lieuArrive))
  }

  // ── LA MACHINE LENTE : bridage du fil principal, par CDP ──────────────────
  if (CPU > 1) {
    await client.send('Emulation.setCPUThrottlingRate', { rate: CPU })
    sortie.cpu = CPU
    console.log('CPU bride ×' + CPU)
  }
  ...
  // ══════════ ATTAQUE — LES GESTES QUE PERSONNE N'A MESURÉS ═════════════════

  const clicAuCentre = async (etiq) => page.evaluate((m) => {
    const el = window.__exp.renderer.domElement
    const r = el.getBoundingClientRect()
    const x = r.left + r.width / 2, y = r.top + r.height / 2
    for (const type of ['pointerdown', 'mousedown', 'pointerup', 'mouseup', 'click']) {
    ...
    window.__d16.marque = m
  }, etiq)

  if (SCENARIO === 'clic-suite') {
    // ⚡ **LA DESCENTE ENTIÈRE AU CLIC — jamais mesurée.** Un clic ne descend
    // que d'un palier de `DIVE_TIERS` ; on enchaîne donc les clics jusqu'au
    // crop, en laissant la vue se poser entre deux.
    await poserOrbite(DEPART_M)
    await vider()
    await marque('DEPART-CLIC')
    const etats = []
    for (let i = 0; i < 20; i++) {
      const av = await page.evaluate(() => ({
        mode: window.__exp.modes?.mode, altM: window.__exp.modes?.altM,
        zoom: window.__exp.dem?.zoom, alt: window.__exp.altitudeCadrageM?.(),
      }))
      await clicAuCentre('CLIC' + i)
      await dodo(4500)
      const ap = await page.evaluate(() => ({
        mode: window.__exp.modes?.mode, altM: window.__exp.modes?.altM,
        zoom: window.__exp.dem?.zoom, alt: window.__exp.altitudeCadrageM?.(),
        crop: (() => { const u = window.__exp.globe?.uniforms ?? window.__exp.globe?._uniforms; return u?.uCropOn?.value ?? null })(),
      }))
      etats.push({ i, av, ap })
      if (ap.mode === 'surface' && ap.crop > 0.5 && ap.alt < ARRIVEE_M) break
      if (i > 1 && ap.mode === 'surface' && ap.zoom === av.zoom && Math.abs((ap.alt || 0) - (av.alt || 0)) < 1) break
    }
    await dodo(ATTENTE_MS)
    sortie.clicEtats = etats
    const brut = await vider()
    sortie.clicSuite = analyse(brut)
    sortie.brutClicSuite = brut.map(({ img, ...r }) => r)
  }

  if (SCENARIO === 'pano-descente') {
    // ⚡ **DEUX GESTES EN MÊME TEMPS** — molette ET glissement de la souris.
    await poserOrbite(DEPART_M)
    await vider()
    await marque('DEPART')
    // ⚠️ **LES ÉVÉNEMENTS SYNTHÉTIQUES DE PAGE NE SUFFISENT PAS** : le premier
    // essai a rendu 44 `NotFoundError: setPointerCapture` et la trace était
    // indiscernable d'une descente sans glissement. On passe donc par CDP
    // (`Input.dispatchMouseEvent`), qui fabrique des événements de confiance.
    const sx = 640, sy = 400
    const glisse = async (dx, dy) => {
      ...
      for (let k = 1; k <= pas; k++) {
        await client.send('Input.dispatchMouseEvent', { type: 'mouseMoved', x: sx + (dx * k) / pas, y: sy + (dy * k) / pas, button: 'left', buttons: 1 })
        // ⚡ **DEUX GESTES EN MÊME TEMPS** : la molette tombe PENDANT le glissement.
        if (k === 3 || k === 6) await molette(-120, 900 + k)
        await dodo(12)
      }
      await client.send('Input.dispatchMouseEvent', { type: 'mouseReleased', x: sx + dx, y: sy + dy, button: 'left', buttons: 0, clickCount: 1 })
    }
    for (let i = 0; i < CRANS; i++) {
      await molette(-120, i)
      // un glissement toutes les 4 images de molette, PENDANT la descente
      if (i % 4 === 2) { await marque('GLISSE' + i); await glisse(60, 25) }
      await dodo(PERIODE)
      const fini = await page.evaluate((seuil) => {
        const x = window.__exp
        const u = x.globe?.uniforms ?? x.globe?._uniforms ?? null
        return x.modes?.mode === 'surface' && (u?.uCropOn?.value ?? 0) > 0.5 && (x.altitudeCadrageM?.() ?? 1e9) < seuil
      }, ARRIVEE_M)
      if (fini) { await marque('CROP-ATTEINT'); break }
    }
    await dodo(ATTENTE_MS)
  ...
  if (SCENARIO === 'resize-descente') {
    // ⚡ **LE REDIMENSIONNEMENT PENDANT UNE DESCENTE** — jamais essayé.
    await poserOrbite(DEPART_M)
    await vider()
    await marque('DEPART')
    const tailles = [[1024, 640], [1280, 800], [800, 900], [1440, 700], [1280, 800]]
    let ti = 0
    for (let i = 0; i < CRANS; i++) {
      await molette(-120, i)
      if (i % 12 === 6) {
        const t = tailles[ti++ % tailles.length]
        await marque('RESIZE' + t[0] + 'x' + t[1])
        await page.setViewport({ width: t[0], height: t[1] })
      }
      await dodo(PERIODE)
      const fini = await page.evaluate((seuil) => {
        const x = window.__exp
  ...
  if (SCENARIO === 'va-et-vient') {
    // ⚡ **L'ALLER-RETOUR RAPIDE** — réserve ③ de l'étape ① du rapport D16 :
    // « sortir plus haut de 38,6 % rapproche la porte de replongée […] non
    // mesuré sur un aller-retour rapide ». On ne laisse RIEN se poser.
    await poserOrbite(DEPART_M)
    await vider()
    await marque('DEPART')
    const sens = [-120, 120, -120, 120, -120]
    for (let tour = 0; tour < sens.length; tour++) {
      await marque(sens[tour] < 0 ? 'DESCENTE' + tour : 'MONTEE' + tour)
      for (let i = 0; i < CRANS; i++) {
        await molette(sens[tour], i)
        await dodo(45) // ⚠️ MOITIÉ de la période nominale : on se dépêche
        const fini = await page.evaluate((seuil, versLeBas) => {
          const x = window.__exp
          const u = x.globe?.uniforms ?? x.globe?._uniforms ?? null
          if (versLeBas) return x.modes?.mode === 'surface' && (u?.uCropOn?.value ?? 0) > 0.5 && (x.altitudeCadrageM?.() ?? 1e9) < seuil
          return (x.modes?.altM ?? 0) > 3e7 && x.modes?.mode === 'orbital'
        }, ARRIVEE_M, sens[tour] < 0)
        if (fini) break
      }
      // ⛔ AUCUNE PAUSE ENTRE LES DEUX SENS : c'est tout l'objet du scénario.
    }
    await dodo(ATTENTE_MS)
    const brut = await vider()
    sortie.descente = analyse(brut)
  ...
  if (SCENARIO === 'z16') {
    // ⚡ **LE CROP ABSENT À z16 APRÈS `gotoCtl.go` — réserve ① de D16-c.**
    // On rejoue le geste exact, on photographie, et on compte les pixels non
    // noirs : « écran vide » est une affirmation, `pixels != fond` est une mesure.
    const releve = []
    const photo = async (nom) => {
      const b = await client.send('Page.captureScreenshot', { format: 'png' })
      fs.writeFileSync(path.join(dossierImg, nom + '.png'), Buffer.from(b.data, 'base64'))
    }
    const etat = async (quoi) => {
      const e = await page.evaluate(() => {
        ...
        const u = x.globe?.uniforms ?? x.globe?._uniforms ?? null
        return {
          mode: x.modes?.mode, busy: !!x.modes?.busy, travel: !!x.modes?.travel,
          zoom: x.dem?.zoom, demZoom: x.params?.demZoom, lat: x.dem?.lat, lon: x.dem?.lon,
          emprise: x.dem?.extentMeters, size: x.dem?.size,
          alt: x.altitudeCadrageM?.(), camY: x.camera?.position?.y,
          crop: u?.uCropOn?.value ?? null,
          cropPose: !!x.veilleCrop?.pose, cropRefus: x.veilleCrop?.refus ?? null,
          signature: x.veilleCrop?.signature ?? null,
          bascules: x.veilleCrop?.bascules ?? null,
          annonce: document.querySelector('#announce, .announce, [class*=announce]')?.textContent?.slice(0, 90) ?? null,
          // la moyenne de l'image telle que l'instrument la voit
          lum: window.__d16?.dernier ? window.__d16.dernier.reduce((s, v) => s + v, 0) / window.__d16.dernier.length : null,
          nonNoir: window.__d16?.dernier ? window.__d16.dernier.filter((v) => v > 8).length : null,
          taille: window.__d16?.dernier?.length ?? null,
        }
      })
      releve.push({ quoi, ...e })
      console.log(quoi, JSON.stringify(e).slice(0, 300))
      return e
    }
    await etat('00-depart')
    await photo('00-depart')
    // 1) le geste de D16-c, tel quel
    sortie.z16go = await page.evaluate(async (t) => {
      try { return await window.__exp.gotoCtl.go(t) } catch (er) { return 'err:' + er.message }
    }, opt('--vers', '45.8326, 6.8652'))
    for (const s of [5, 15, 30, 60]) {
      await dodo(s === 5 ? 5000 : 10000 * (s === 15 ? 1 : s === 30 ? 1.5 : 3))
      await etat(`01-apres-go-${s}s`)
      await photo(`01-apres-go-${s}s`)
    }
    // 2) est-ce le z16, ou est-ce `gotoCtl.go` ? un contre-essai à z12.
    sortie.z12go = await page.evaluate(async () => {
      try { return await window.__exp.modes.flyTo(45.8326, 6.8652, 12) } catch (er) { return 'err:' + er.message }
    })
    await dodo(15000)
    await etat('02-flyto-z12')
    await photo('02-flyto-z12')
    // 3) et le même z16 atteint par flyTo direct (sans gotoCtl) ?
    sortie.z16flyto = await page.evaluate(async () => {
      try { return await window.__exp.modes.flyTo(45.8326, 6.8652, 16) } catch (er) { return 'err:' + er.message }
    })
    await dodo(20000)
    await etat('03-flyto-z16')
    await photo('03-flyto-z16')
    // 4) et un z16 atteint À LA MOLETTE depuis le z12 ? (le chemin de l'usager)
    sortie.z12retour = await page.evaluate(async () => {
      try { return await window.__exp.modes.flyTo(45.8326, 6.8652, 12) } catch (er) { return 'err:' + er.message }
    })
    await dodo(15000)
    await etat('04-retour-z12')
    for (let i = 0; i < 60; i++) { await molette(-120, i); await dodo(120) }
    await dodo(6000)
    await etat('05-molette-depuis-z12')
    await photo('05-molette-depuis-z12')
    sortie.z16 = releve
    sortie.z16lignes = analyse(await vider())
  }
