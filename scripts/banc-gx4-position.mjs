// GX4 — LA POSITION ABSOLUE, CONTRE LA SURFACE QUE LE GPU DESSINE. C'est le
// banc de GX3 (`banc-gx3-position.mjs`) à une ligne près : sa « vérité » posait
// le point à `hauteurDessinee` — les hauteurs RÉSERVÉES, celles du zoom du MNT
// (z10 au cadrage z10), pas celles des tuiles que le crop dessine (z12/z13,
// encore en cache et allumées). Depuis GX4 le ruban suit le MAILLAGE dessiné ;
// mesuré contre l'ancienne vérité il en paraît à 288 m (col 1, z10) alors qu'il
// est à 2 m de la surface rendue. Ici la vérité est `hauteurMaillee` sur les
// tuiles allumées dans le socle (la loi de GX4), et l'ancienne est rapportée à
// côté (`hDessReservee`), avec l'écart au rayon (surface RENDUE) du sommet.
// EMPLOI : node scripts/banc-gx4-position.mjs [--gpx x] [--etiquette mb] [--zooms 10,11,12,13]
...
await page.evaluate(() => {
  ...
  window.__verite = (p) => {
    ...
    const hReservee = g.hauteurDessinee(p.lat, p.lon, liste)
    const dessinees = []
    for (const t of g.tiles.values()) if (t.mesh?.visible && t.mesh.geometry?.attributes?.position) dessinees.push(t)
    dessinees.sort((a, b) => b.z - a.z); dessinees.trieeFinAbord = true
    const hDess = g.hauteurMaillee(p.lat, p.lon, dessinees) ?? hReservee
    ...
    if (best != null && ws[best]) {
      ...
      // l'écart du sommet à la surface RENDUE, au rayon (comme banc-gx3-drapage)
      ...
      const rc = new T.Raycaster(); rc.near = 0; rc.far = 30; const dir = s.clone().normalize(); rc.set(dir.clone().multiplyScalar(110), dir.clone().negate())
      let rMax = -1, tuile = ''; for (const h of rc.intersectObjects(meshes, false)) { const r = h.point.length(); if (r > rMax) { rMax = r; tuile = h.object.name } }
      produit = { i: best, px: (qs.x * 0.5 + 0.5) * W, py: (-qs.y * 0.5 + 0.5) * H, rayon: s.length(), dLatLonM: bd * 111320, ecartRenduM: rMax < 0 ? null : +((s.length() - rMax) * (EARTH_RADIUS_M / R_GLOBE) / exag).toFixed(1), tuile }
    }
    return { hDess, hReservee, source: hDess != null ? 'globe' : 'gpx', px, py, dansEcran, distU, mParPx, dansCrop, rayonVerite: v.length(), produit, demZoom: e.params.demZoom, alt: e.altitudeCadrageM?.() }
  }
})
...
for (const z of ZOOMS) {
  for (const p of points) {
    ...
    const r = await releve(`${ETIQ}-gx4pos-z${z}-${p.nom.replace(/\s/g, '')}`, { image: true })
    ...
    const ligne = { zoom: z, point: p.nom, alt: Math.round(v.alt), demZoom: v.demZoom, dansCrop: v.dansCrop, source: v.source, hDess: v.hDess, hReservee: v.hReservee, ecartRenduM: v.produit?.ecartRenduM ?? null, tuile: v.produit?.tuile ?? null, ele: p.ele, dansEcran: v.dansEcran, verite: [Math.round(v.px), Math.round(v.py)], dPx, dM: dPx == null ? null : +(dPx * v.mParPx).toFixed(1), mParPx: +v.mParPx.toFixed(2), sommetProduitPx: dProduitPx == null ? null : +dProduitPx.toFixed(1), sommetProduitM: dProduitPx == null ? null : +(dProduitPx * v.mParPx).toFixed(1), ecartLatLonM: v.produit ? +v.produit.dLatLonM.toFixed(0) : null, rayonVerite: v.rayonVerite, rayonProduit: v.produit?.rayon, pixels: r.pixels, bruit: r.bruit, repos: rp.repos }
    R.echelles.push(ligne)
    console.log(`  z${z} ${p.nom.padEnd(8)} alt=${ligne.alt} m  crop=${v.dansCrop ? 'DANS' : 'HORS'}  écran=${v.dansEcran}  vérité=(${Math.round(v.px)},${Math.round(v.py)})  tracé le + proche : ${dPx == null ? '—' : dPx.toFixed(1) + ' px = ' + ligne.dM + ' m'}  (sommet produit : ${ligne.sommetProduitPx ?? '—'} px = ${ligne.sommetProduitM ?? '—'} m, Δr=${v.produit ? ((v.produit.rayon - v.rayonVerite) * 63710 / 2).toFixed(1) : '—'} m)  pixels=${r.pixels} bruit=${r.bruit}  ${v.mParPx.toFixed(1)} m/px  h dessinée=${v.hDess?.toFixed(0)} (réservée ${v.hReservee?.toFixed(0)}) · écart rendu du sommet ${ligne.ecartRenduM} m (${ligne.tuile})`)
  }
}
fs.writeFileSync(path.join(SORTIE, `${ETIQ}-gx4-position.json`), JSON.stringify(R, null, 1))
await B.nav.close()
