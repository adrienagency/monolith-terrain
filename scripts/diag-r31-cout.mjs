// DIAG R31 — LE COÛT GPU DU RECOLLAGE, À LA MINUTERIE DU PILOTE
//
// ⚠️ **PROTOCOLE REPRIS DE `diag-r28-cout.mjs` SANS EN CHANGER UNE RÈGLE** :
// minuterie du pilote, témoin de validité par ×16 fragments, 40 rendus de chauffe
// après CHAQUE recompilation, ordre A/B tournant, différences APPARIÉES et
// médiane. Seule la substitution change — ici la variante « sans » retire la
// LIGNE de recollage, et rien d'autre.
//
...
function poserBanc() {
  ...
  // ⚠️ **UNE SEULE SUBSTITUTION, ET C'EST TOUT L'INTÉRÊT** : `rampTMonde` reste
  // calculé (le mélange `dedansCrop` en a besoin), donc ce que la variante « sans »
  // retire est EXACTEMENT le `mix` de recollage — deux ALU. Toute autre
  // substitution aurait mesuré autre chose que la tâche.
  const SANS = [
    ['rampT = mix(rampT, rampTMonde, uRecollage);', '// recollage retire (banc R31)'],
  ]
  const materiaux = () => {
    const out = []
...
try {
  ...
  for (const L of LIEUX) {
    ...
    else await page.evaluate((a) => window.__exp.modes._rescale(a, 'BANC R31'), L.aller)
    ...
    console.log(`[${L.nom}] alt=${Math.round(info.altM)} m crop=${info.crop} tuiles=${info.tuiles} materiaux=${info.materiaux}`
      ...
      + `\n   AVEC recollage ${ligne.A.moy.toFixed(4)} ± ${ligne.A.sigma.toFixed(4)} ms (n=${ligne.A.n})`
