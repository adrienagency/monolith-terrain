...
const t0 = P.tDebut
const cle = (r) => [r.demZoom, r.pose, r.cropDemi, r.paroisDemi, r.provisoire, r.parois, r.cropSeul, r.refus, r.dehorsPermis, r.armee, r.auRepos, r.busy, r.annonce, r.mode, (r.horsPx ?? 0) > 0, r.dessineesHors > 0, r.uEstompage >= 0.999, r.uEstompage <= 0.001].join('|')
let prev = null
const lignes = []
for (const r of rec) {
