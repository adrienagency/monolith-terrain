...
- [ ] **Étape 1 — DÉVERROUILLER `this.busy` ET `demBusy`. ⚠️ CE PLAN N'AVAIT ICI QUE DES CONSTATS, SANS UN SEUL VERBE À L'IMPÉRATIF.**
  ...
  - ⚠️ **`demBusy` — SECOND VERROU, ZÉRO MENTION DANS TOUTES LES VERSIONS PRÉCÉDENTES, ET IL EN VAUT DEUX.** Six sites dans `main.js` (`:3057`, `:3384-3385`, `:3413`, `:3602-3603`, `:3619`), **avec deux contrats opposés sur deux chemins différents** : `:3384` fait un `return` silencieux, `:3602` lève `throw new Error('terrain busy')` → **bandeau « FAILED » à l'écran**. Il garde aussi `canStep` (`main.js:9475`) **avec** `modes.busy`. **Traitez les deux chemins séparément : le silencieux et le bruyant n'ont pas le même remède.**
  ...
  - **Le seuil, et ⚠️ CE PLAN EN A ÉCRIT DEUX QUI NE MORDENT PAS.** « `busy || demBusy` vrai, image par image » : un verrou qui met en file **reste vrai**. « Zéro geste refusé » : **la file, la cible et l'absence totale de verrou marquent toutes les trois zéro.** ⚠️ **Le critère qui distingue les trois est la LATENCE et la TRAÎNE** : latence médiane entre le geste et son effet **sous 10 images**, et **traîne après le dernier geste sous 0,5 s**. Mesurez les trois régimes et comparez.
  - ⚠️ **ET N'ANNONCEZ PAS « L'ENTRÉE MORTE SOUS 1 s » : C'EST ARITHMÉTIQUEMENT HORS D'ATTEINTE ICI.** `_whiteout` fait 480 + 480 ms et il est **attendu avant `busy = false`** : il pose à lui seul un plancher de **6,77 s même avec un réseau instantané**. Et le §10 le retire **en dernier**. Une fois parti, il faudrait encore que `loadSurface` tombe de 1 000 ms à **≈143 ms**, sur un chemin qu'aucune tâche ne touche. **Le seuil de cette étape est la latence et la traîne, pas le total.**
...
- [ ] **Étape 3 — TRANCHER LA QUESTION OUVERTE, et ⚠️ LE FAIT QUI LA TRANCHE PRESQUE EST DÉJÀ DANS LE DÉPÔT.** `dem-source.js:39` donne `baseZoom: 12` à Mapterhorn — **son plancher de couverture est z12, et le globe plafonne à z11.** **La sonde par zone ne peut donc RIEN changer à ce que le globe orbital dessine**, tout en coûtant **six requêtes HEAD par zone z8, hors `MAX_CONCURRENT`** — de l'ordre de **2 520 pour une vue orbitale**. ⚠️ **Et `coast-mask.js` répond gratuitement à la seule question qui compte à ces zooms : terre ou mer.** La question devient donc : **rebrancher seulement SOUS z12, là où le socle vit**, et laisser le globe orbital sur AWS. **Mesurez, puis écrivez la décision dans cette case.**
...
- [ ] **Étape 3 — implémenter les trois corrections, ET LES CINQ INTERFACES QUE CETTE TÂCHE DÉCLARE. ⚠️ CE PLAN EN DÉCLARAIT SIX ET N'EN FABRIQUAIT QU'UNE.** `creerFlux`, `tuilesPretes`, `zoomEffectif`, `remplirHauteurs` et `demanderEmprise` n'apparaissaient **qu'une seule fois dans tout le document : leur déclaration.** Aucune case ne les écrivait, aucun test ne les touchait. **Une case par interface, avec son assertion.** ⚠️ **Et l'audit de noms du §10 était aveugle exactement à celles-là : ajoutez-les-y.**
