...
- ~~**Le régime `?socle=quadtree` n'a pas été essayé avec la frontière.**~~ ✅ **ESSAYÉ APRÈS COUP, ET IL TIENT.** `?globe=continu&socle=quadtree&f3=0&frontiere=1`, La Réunion : la fenêtre bornée est active (`largeurM` = 27 354 m), la chaîne est bien `[PasseFond, ClearPass, RenderPass, EffectPass]`, et le raccord passe par la branche `mondeVersLatLonEmprise` de `latLonOrigineBloc()` — **écart 0,03 px à l'ancre, 0,21 px à 18 km**, les mêmes chiffres que sous `?globe=crans`. ⚠️ **Ce qui reste non vu dans ce régime : la descente complète, cran par cran.**
...
### Tâche 2 : retirer la carte `#loading` ✅ **FAITE LE 2026-08-21** — ⚠️ **ELLE PART SOUS LE DRAPEAU, PAS EN PRODUCTION — ET IL Y AVAIT DEUX LEVÉES DE RIDEAU, PAS UNE**
...
- [x] **Étape 1 — le test qui échoue.** → `test/voile-loading.test.js`, **12 tests**. Assertion de texte source pour `main.js` et `style.css` ; la moitié pure de l'indicateur (`texteRetard`) est testée **en exécution**.
- [x] **Étape 2** — lancé au niveau de `d2f00e3` : **3 rouges**, dont les deux demandées.
- [x] **Étape 3 — retirer l'appel du chemin de zoom.** → **il y en avait DEUX, et ce plan n'en nommait qu'un** — voir le bilan.
- [x] **Étape 4 — `ld-warm`.** → **elle ne bouge pas, et elle ne disparaît pas** : le flou parti, il lui reste son emploi d'origine (`body.ld-warm #loading-bg { opacity: 0 }`). Vérifié à l'écran.
- [x] **Étape 5 — mutation** : trois, une par correctif, chacune tue exactement une assertion.
- [x] **Étape 6 — LA CLÔTURE DU §0**, les quatre commandes dans l'ordre, puis commit `26639b3`.

#### Bilan de la Tâche 2 — ce qui est parti, ce qui reste, et pourquoi

⚠️ **IL Y AVAIT DEUX LEVÉES DE RIDEAU SUR LE CHEMIN DU ZOOM, ET CE PLAN N'EN NOMMAIT QU'UNE.** `fetchAndBuildDem` lève la carte lui-même (`if (!enVol) showLoading()`, déjà borné par la Tâche 6 septies) **puis `regenerateTerrain` la relève une seconde fois**, sans rien savoir du vol. **Mesuré à l'écran** (port 5503, `?globe=continu&socle=quadtree&f3=0`, La Réunion, cran z12 → z13, observateur de mutations sur la classe de `#loading`) : `loadSurface` rend la main en **468 ms**, la carte remonte **326 ms plus tard** par-dessus une application déjà libre, et elle était **encore là 9,7 s après**. → `await regenerateTerrain({ sansRideau: enVol })`. **Après, même cran : zéro levée**, `loadSurface` en 627 ms.

⚠️ **ET HORS DRAPEAU LE RIDEAU RESTE, DÉLIBÉRÉMENT.** `socleQuadtree` est éteint en production : `volPossible()` y rend faux et `loadSurface` y bloque encore — **mesuré, `?globe=crans`, même cran : 2 605 ms**. L'attente n'a pas disparu de ce côté-là, et le §10 l'écrit mot pour mot. **La même ligne fera tomber le rideau le jour où le drapeau s'allumera.**

**LE FLOU À L'ARRÊT — le seul changement de production, et c'est un défaut des deux côtés.** Mesuré avant d'être affirmé, transitions neutralisées pour lire l'état d'arrivée : avec `body.ld-warm` posée et `#loading` visible, `getComputedStyle(#app)` rendait **`blur(16px)`** et **`matrix(1.04, 0, 0, 1.04, 0, 0)`**, pendant les **1 177 ms** d'un simple rechargement. Après : `none` et `none`. La transition `filter/transform` sur `#app` part avec — elle n'animait que ce flou-là.

**LES QUATRE EFFETS DE BORD DE `hideLoading`, UN PAR UN :**

| | sort | pourquoi |
|---|---|---|
| `ld-warm` | **reste** | le flou parti, il lui reste `#loading-bg { opacity: 0 }` — son emploi d'origine. Vérifié à l'écran : `bgOpacity` vaut toujours 0 |
| `globe.chargeRacines()` | **inchangé** | la Tâche 1b avait déjà posé `assureRacinesGlobe` (idempotent) et son filet à 20 s. **Les deux demandeurs sont épinglés** par le test ④ : les perdre ne lève aucune erreur |
| plancher `LOADING_MIN_MS` | **gardé** | il ne s'applique qu'au PREMIER congé. Le premier affichage est peint **en ligne par index.html**, sans passer par `showLoading` — `index.html` n'a pas été touché |
| les 2,6 s du `catch` | **retirées** | le `finally` relâche `demBusy` tout de suite : l'application était libre et l'écran disait le contraire. C'est la place que le §9 désigne |

**L'INDICATEUR DISCRET EST DESSINÉ** — `src/ui/indicateur-retard.js` + `.ce-retard`. Non bloquant, `role="status"`, **en niveaux jamais en pourcentage**, **éteint quand la mesure manque**. ⚠️ **Il n'est PAS nourri par `etatIndicateur`, et c'est la même mesure qui l'interdit que celle qui avait chassé `remplirBorne` de ce chemin** : sur un lien **oisif**, `debitObserve` rendait 0,787 Mb/s et `zoomSoutenable` en tirait z5 pour une demande de z12 — l'indicateur serait resté allumé en permanence sur une connexion parfaite. On lui passe donc un **fait** (`zoomEffectif`, la couverture réellement disponible), et `null` l'éteint. La forme `{enRetard, niveaux}` est celle d'`etatIndicateur` : le jour où la Tâche 1 branchera R3 sur la caméra, son retour se passera ici tel quel. **Vu à l'écran** : allumé à 596 ms (« détail en cours… 1 niveau de retard »), **éteint seul à 1 067 ms**.

**LA QUESTION DE LA TÂCHE 7, TRANCHÉE** — *« où va la seconde et demie une fois le voile parti ? »* **Elle ne bouge pas.** Le préchauffage des deux finesses reste exactement où il est, dans `regenerateTerrain`, et le voile est toujours levé **là où il l'était** : au premier chargement, et sur le chemin de zoom **en production**. Ce qui change est le chemin de **vol**, qui n'attendait déjà plus — la 1 516 ms de cuisson y est déjà hors du gel perçu (Tâche 6 septies). **Il n'y a donc pas de seconde et demie à reloger tant que le drapeau est éteint** ; le jour où il s'allumera, c'est la Tâche 7 qui devra la porter, et elle le sait.

⚠️ **UN TEST EXISTANT CORRIGÉ EN PLACE, PAS REMPLACÉ.** `test/terrain-jobs.test.js` cherchait `await regenerateTerrain()` **mot pour mot** pour garder l'ORDRE du lâcher de trait de côte. Le motif devient `await regenerateTerrain(` — l'appel, pas ses arguments (`grep -c` = 1 dans `main.js`).

⚠️ **CE QUI N'A PAS PU ÊTRE VÉRIFIÉ.** Le volet navigateur de cette machine **ne composite pas** : `requestAnimationFrame` est gelé, **aucune image n'a été vue**. Tout ce qui est mesuré ci-dessus l'est par le DOM et les styles calculés. Et le **chemin de panne réseau** (l'indicateur qui remplace les 2,6 s) n'a pas pu être déclenché à l'écran — trois tentatives, le MNT arrivait à chaque fois ; il n'est couvert que par le test ⑥.

### Tâche 2 ter : retirer le fondu blanc `.whiteout` ✅ **FAITE LE 2026-08-21** — ⚠️ **LA CLAUSE DE SORTIE S'APPLIQUE : LE RIDEAU DU ZOOM EST DÉJÀ PARTI, ET LES TROIS SURVIVANTS RESTENT SUR MESURE**
...
- [x] **Étape 1 — le test** : `test/voile-whiteout.test.js`, **6 tests**, assertion de texte source.
- [x] **Étape 2** — lancé : **il ne peut plus échouer**, et pas pour la raison prévue. La clause de sortie parlait d'appelants **disparus** ; ici les trois sont **bien là**, mais **aucun n'est sur le chemin du zoom continu** — le seul qui y était (`_rescale`) est parti avec la Tâche 2 bis. **C'est une bonne nouvelle, pas un test cassé** : le fichier est gardé comme cliquet.
- [x] **Étape 3 — les TROIS appelants tranchés : ILS RESTENT, TOUS LES TROIS**, et trois mesures l'imposent — voir le bilan. **L'indicateur discret n'est PAS l'un d'eux** : il a été dessiné pour de bon à la Tâche 2 (`src/ui/indicateur-retard.js`), donc la première question du §9 n'est plus sans réponse et ce rideau n'a pas à en tenir lieu.
- [x] **Étape 4 — mutation** : trois — remettre un rideau dans `_rescale`, en perdre un des trois, changer le prix de 480 ms. Chacune tue.
- [x] **Étape 5 — LA CLÔTURE DU §0**, les quatre commandes dans l'ordre, puis commit.

#### Bilan de la Tâche 2 ter — les trois survivants, tranchés sur mesure

**Relevés le 2026-08-21 sur l'application vivante** (port 5503, `?globe=crans`, La Réunion), pas déduits de la lecture :

| appelant | ce que le rideau masque, MESURÉ | sort |
|---|---|---|
| `enterOrbit` | `globe.visible` bascule **false → true** dans la même image où le terrain fait l'inverse ; `camera.far` **290 → 1 400** ; la caméra saute de **(88,49 · 72,72 · 88,49)** — au-dessus d'une dalle à l'origine — à **(77,24 · −36,33 · 52,56)**, un point d'une **sphère de rayon `R_GLOBE`**. **Elle traverse le plan y = 0** | **reste** |
| `_dive` | le même échange, dans l'autre sens. `src/modes.js` le dit à l'endroit exact : « le repère de POSITION change bel et bien ; c'est l'Étape 2, la frontière globe/terrain, et **elle n'est pas faite** » | **reste** |
| `_loadDive` | il ne change pas de monde — c'était le seul qui aurait pu partir. Il **téléporte encore** au point de présentation : **z12 → z13, l'altitude passe de 3 622 m à 6 680 m, ×1,844 — la caméra MONTE pendant que l'utilisateur clique pour descendre** (cadrage ×1,984, distance ×5,34 : 26,7 → 142,8 unités). ⚠️ **Et c'est une borne basse** : la mesure a appelé `_loadDive` seul, sans le premier temps de `diveTo` (l'approche de 30 %) | **reste** |

⚠️ **POURQUOI `_loadDive` N'A PAS REÇU LE TRAITEMENT DE `_rescale`.** Sa pose d'arrivée n'est pas un accident : c'est une demande d'Adrien **citée dans le code** — « dézoomé quasiment au max de ce niveau, même axe de vue ». La corriger serait renverser en passant une décision d'utilisateur écrite. **C'est une question pour Adrien, pas une étape de cette tâche** — ajoutée au §9.

**La condition de sortie est écrite comme assertion.** Le test épingle `globe.setVisible` / `setSurfaceVisible` dans `enterOrbit` et `_dive`, et `distancePresentation` dans `_loadDive` : **le jour où la Tâche 1b bis les fera disparaître, ces tests tomberont — et ce sera le signal que les rideaux peuvent partir**, pas une régression. Le prix du rideau (480 + 480 ms) est épinglé lui aussi : c'est sur lui que l'arbitrage se fait, il ne doit pas changer en silence.

⚠️ **`src/style.css` N'A PAS ÉTÉ TOUCHÉ** (`.whiteout`, `:590-602`) : le rideau a encore trois appelants, sa feuille reste.
...
- ~~**Rien n'est branché.**~~ ⚠️ **`socleVisible` EST BRANCHÉ DEPUIS LE 2026-08-21 — Tâche 3 bis ci-dessous.** La ligne reste écrite parce qu'elle a été VRAIE pendant une journée, et que c'est ce que **Adrien a vu à l'écran** : à Z5, un socle posé devant la Terre entière. `empriseSocle`, lui, n'est toujours lu que pour NOMMER l'emprise de la fenêtre (Tâche 6 ter).

### Tâche 3 bis : BRANCHER LE SEUIL DU SOCLE ✅ **FAITE LE 2026-08-21** — ⚠️ **DEUX DÉFAUTS TROUVÉS À L'ÉCRAN, ET AUCUN DES DEUX N'ÉTAIT DANS LE SEUIL**

**Fichiers :** `src/monde/veille-socle.js` (créer) · `src/main.js` · `src/flags.js` · `src/globe.js` · `test/seuil-branche.test.js` (créer, **22 tests**, inscrit à la ligne `test` — **193 fichiers, `audit:tests` sans écart**)

⚠️ **CETTE TÂCHE N'EXISTAIT PAS DANS LE PLAN, ET C'EST LE DÉFAUT QUE LE §10 DÉCRIT :** la Tâche 3 avait livré `socleVisible`, testé et muté, et son compte rendu se terminait par « **Rien n'est branché** ». Personne ne l'a branché. **Adrien l'a signalé à l'écran, capture à l'appui** : *« j'ai l'impression que tu calcules 2 fois la map, l'une avec la terre complète en arrière-plan et l'autre avec le socle. »* Il avait raison, et la cause tenait en une ligne de `grep` : `grep -rn "socleVisible" src/main.js` ne rendait **rien**.

**Interfaces produites :** `creerVeilleSocle({ appliquer, socleAuDepart, modeSurfaceAuDepart })` → `{ maj, poserMode, visible, auSeuil, bascules }` · `socleAffiche()` (`main.js`) · `seuilSocleActif()` (`flags.js`) · `FLAGS.seuilSocle`

- [x] **Étape 1 — le test qui échoue** : à 4 000 km le socle n'est pas posé, sous le seuil de naissance il l'est ; **cent oscillations, UNE bascule**. — 22 tests.
- [x] **Étape 2** — le lancer, vérifier qu'il échoue. — `ERR_MODULE_NOT_FOUND` sur `src/monde/veille-socle.js`.
- [x] **Étape 3 — brancher.** L'entrée est `altitudeCadrageM()`, l'altitude géométrique **sans `dem.meanM`** — règle R1.
- [x] **Étape 4 — ce qui disparaît avec le socle.** ⚠️ **UN ORPHELIN TROUVÉ, ET SEULEMENT À L'ÉCRAN.**
- [x] **Étape 5 — mutation** : trois, toutes mordantes.
- [x] **Étape 6 — REGARDER L'ÉCRAN.** Fait, et c'est là que les deux vrais défauts sont sortis.
- [x] **Étape 7 — LA CLÔTURE DU §0**, les quatre commandes dans l'ordre, puis commit.

#### ✅ CE QUI A ÉTÉ FAIT, ET CE QUE ÇA MESURE (2026-08-21)

**① LA LOI N'A PAS BOUGÉ D'UN BIT — ON N'A AJOUTÉ QU'UNE MÉMOIRE.** `seuil-socle.js` est intouché (ses 25 tests aussi). `socleVisible` est une FONCTION, donc sans mémoire, et son compte rendu le dit : « `visibleAvant` n'est pas un confort : c'est l'hystérésis ». `src/monde/veille-socle.js` porte cet état, plus trois choses que la fonction ne pouvait pas porter : le **ET avec le mode**, l'**application sur changement seulement**, et le **gel en orbite**.

⚠️ **LE GEL EN ORBITE N'EST PAS UNE ÉCONOMIE, C'EST UNE CORRECTION, ET ELLE EST MESURÉE.** En mode orbital la caméra vit sur la sphère de rayon 100, pas au-dessus de la dalle : relevé au navigateur, `altitudeCadrageM()` y rend **−523 km** puis **−850 km** — des altitudes NÉGATIVES. Une altitude négative passe sous les deux seuils : sans le gel, l'orbite ferait naître le socle.

**② LES DEUX SEUILS, ET OÙ ILS TOMBENT VRAIMENT.** ⚠️ **LE PLAN AVAIT UNE COÏNCIDENCE DANGEREUSE QUE PERSONNE N'AVAIT REGARDÉE :** l'**ancre à 60 %** de z13 vaut **32,27 km**, c'est-à-dire `SEUIL_NAISSANCE_M` **au mètre près** — si le branchement avait donné l'ancre à l'automate, le socle serait né pile sur sa frontière. Ce que `main.js` donne est l'altitude RÉELLE de la caméra, et elle est cinq fois plus basse que l'ancre parce que la pose d'arrivée est oblique et proche. Rejeu (`.banc/rejeu-arrivee.mjs`, `distanceArrivee(150)` = 141, pente 18/19 → `camY` = 96,972, 45°) :

| | z4 | z5 | z10 | **z11** | z13 | z15 |
|---|---|---|---|---|---|---|
| ancre à 60 % | 16 524 km | 8 262 km | 258 km | 129 km | **32,27 km** | 8,07 km |
| **altitude de cadrage réelle** | 3 680 km | **920 km** | **51,3 km** | **25,7 km** | 6,42 km | 1,60 km |

**La bascule tombe entre z10 et z11, à mi-chemin des deux seuils : aucune pose d'arrivée ne les frôle.** Dans un même palier le socle peut naître ou mourir au dolly — à z10, naissance à 88,6 unités de distance, mort à 110,8 — **et c'est le geste continu qu'Adrien décrit**, pas un cran.

**③ ⚠️ LE CRAN EST UN OSCILLATEUR, ET C'EST LE PREMIER DÉFAUT QUE L'ÉCRAN A SORTI.** Première mesure d'une descente : **ONZE bascules** sur sept crans z12 → z5. Le journal par image dit pourquoi, et ce n'est pas le seuil :

| alt lue | socle | zoom | `busy` | largeur | `camY` |
|---|---|---|---|---|---|
| 40 751 m | 1 | 9 | 0 | 219 km | 29,19 |
| **20 375 m** | **0** | 10 | **1** | **109 km** | **29,19** |
| 40 751 m | 1 | 10 | 0 | 109 km | **58,38** |

**`largeurBlocM()` est divisée par deux UNE IMAGE AVANT que `_rescale` ne double `camera.position.y`.** Entre les deux, l'altitude lue vaut exactement la MOITIÉ de la vraie, et la moitié de 40 751 tombe de l'autre côté des deux seuils. ⚠️ **Ce n'est pas une violation de R1, c'est sa confirmation** : `largeurBlocM()` est l'emprise du bloc CHARGÉ, et pendant un cran elle est désaccordée de la caméra. **Le remède n'est pas de changer d'instrument** — `altitudeCadrageM()` reste le seul sans `meanM` — **c'est de ne pas décider pendant que les deux moitiés sont désaccordées.** `modes.busy` marque exactement cet intervalle, mesuré : il est vrai sur toutes les images du transitoire. Seconde garde du même geste : `largeurBlocM() > 0`, parce que sans emprise `altitudeCadrageM()` bascule sur un **tout autre calcul** (`terrain.heightToFeet`, le relief procédural) — l'instant qu'`entrerEnVol` ouvre en posant `dem = null`.

> **Après la garde : UNE bascule sur les sept crans z12 → z5, et UNE sur la remontée z5 → z11.** Trace de la descente, altitude de cadrage par palier : z6 **1 280 km** · z7 828 km · z8 473 km · z9 237 km · z10 118 km · **z11 59,1 km → socle absent**, puis le dolly à 29,6 km **→ socle né, une fois, sans clignoter**.

**④ ⚠️ L'ORPHELIN — ET IL A FALLU L'ÉCRAN, PAS LE CODE.** Socle retiré à Z5, **le cartouche `ground-info` et ses HUIT MAILLES restaient dessinés SUR la planète**. Cause : `poserVisibiliteSocle` n'est **pas le seul écrivain** de la visibilité des calques — **dix-sept lignes de `main.js`** la ré-affirment avec leur propre condition, toujours `modes.mode === 'surface'`, qui ne connaît que le MODE. La pire est dans `fetchAndBuildDem` : elle rallume le cartouche **à chaque palier**. C'est, à la lettre, l'accident de la liste dupliquée que ce fichier raconte cinq fois.

**Le geste : UNE seule réponse, `socleAffiche()`.** ⚠️ **Sans drapeau elle EST l'expression d'avant, au caractère près** (`modes?.mode !== 'orbital'`) : il n'y a que deux modes, et un test l'exige — si un troisième apparaissait, `!== orbital` cesserait d'être `=== surface`. **Vérifié après correction, traversée de scène à Z5, socle retiré : un seul objet dessiné reste, le DISQUE SOLAIRE** — il est du ciel, pas du socle, et il reste allumé en orbite aujourd'hui aussi.

**⑤ ⚠️ LE SECOND DÉFAUT EST DANS LA TÂCHE 1b bis, ET IL RENDAIT LE SOCLE RETIRÉ INUTILISABLE.** Le seuil retirait bien le socle… et découvrait une **boule blanche sans continent**. `_dive` pose `globe.setVisible(false)`, donc `enabled = false`, donc `Globe.update()` **sortait à sa première ligne** — alors que `main.js` l'appelle EXPRÈS en mode surface sous la frontière, avec un commentaire qui prévient que « sans cet appel il reste à ses seize racines ». **L'appel existait et ne faisait rien**, et `setVisible` affirme pourtant en commentaire qu'`enabled` « dit le MODE, pas le dessin ».

> Mesuré (`?globe=crans&frontiere=1`, La Réunion dézoomée à z5, altitude de cadrage **847 km**) : cache du quadtree **16 tuiles — ses seules racines** ; `update()` rend **0**. Lever `enabled` fait passer le cache à **52 tuiles en UNE image**, et la Terre retrouve Madagascar, l'Afrique et ses nuages, au même cadrage.

Correction : `if (!this.enabled && !this.frontiereFond) return 0`. ⚠️ **`frontiereFond` n'est vrai que sous le drapeau** (`main.js` le pose, `globe.js` n'importe pas `flags.js`) : sans drapeau, le garde est celui d'avant, au caractère près. Trois tests le gardent, dont celui du comportement d'avant.

**⑥ CE QU'ON A VU À L'ÉCRAN — Z5, VERS L'AFRIQUE DE L'EST.**

| | ce qui est dessiné |
|---|---|
| **AVANT** (`seuil=0`, altitude de cadrage **847 km**) | le socle z5 — Madagascar, Nairobi, Dar es Salaam, Antananarivo — **posé devant la Terre entière**, avec la gravure « RÉUNION · FRANCE ». **Deux cartes à la fois.** C'est la capture d'Adrien. |
| **APRÈS** (`seuil=1`, altitude de cadrage **1 024 km**) | **la Terre seule**, au même cadrage : Madagascar et la côte est-africaine exactement là où était le socle, océan, nuages. Ni socle, ni mer de socle, ni gravure, ni cartouche, ni rose des vents. |

**⑦ LES TROIS MUTATIONS, TOUTES MORDANTES.**

| mutation | ce qui tombe |
|---|---|
| `RAPPORT_HYSTERESIS = 1` (seuils égalisés) | **les DEUX oscillations** — **200 bascules** au lieu d'une |
| retirer `majSeuilSocle()` de `tick()` | le test de câblage de l'Étape 1 |
| nourrir la veille d'une fraction d'écran au lieu de l'altitude | le test de la règle R1 |

**⑧ ⚠️ CE QUI EST LAID, ET IL FAUT LE DIRE AVANT QU'ADRIEN LE VOIE.**
- **LA MER DU SOCLE ET L'OCÉAN DU GLOBE N'ONT PAS LA MÊME COULEUR.** À la naissance, sur une emprise marine (−16,63 · 50,62), le socle est un pavé **gris pâle** posé sur un océan **turquoise** : la discontinuité au bord du socle est franche et se lit comme un défaut. **Ce n'est pas le seuil, c'est le raccord de palette entre les deux mondes**, et personne ne l'a jamais regardé — la Tâche 1b bis n'avait vu que des images fixes de terre.
- **IL N'Y A AUCUNE TRANSITION.** La décision 4 dit « le socle complet apparaît d'un coup, **avec une transition à dessiner** ». Elle n'est pas dessinée : le socle apparaît brut. Aujourd'hui le rideau du cran le masque à moitié ; le jour où les rideaux tomberont, il sera nu. **La 1b bis a laissé la porte ouverte pour ça** — « un fondu croisé en espace-écran entre les deux passes est trivial » — et c'est là qu'il faut le mettre.
- **L'INDICATEUR DE ZOOM CONTINUE D'AFFICHER « Z5 » QUAND IL N'Y A PLUS DE SOCLE.** Il annonce le palier d'un bloc qu'on ne voit pas. **À trancher avec Adrien** — c'est de la même famille que le bandeau `.fui-msg` de la Tâche 1c, Étape 3 bis.

**⑨ CE QUI N'A PAS PU ÊTRE VÉRIFIÉ.**
- ⚠️ **RIEN N'A ÉTÉ VU EN MOUVEMENT CONTINU.** Les bascules sont comptées par sonde, pas jugées à l'œil : le §10 tient toujours. Et **`requestAnimationFrame` ne se déclenche pas dans un onglet qui ne composite pas** (§3 de `/threejs-optimisation`) — le premier journal n'a récolté que **15 images sur une descente de vingt secondes**, et il a fallu passer par `setInterval` puis ramener l'onglet au premier plan.
- ⚠️ **LE VOL DE RÉFÉRENCE DU §0 N'A PAS ÉTÉ REJOUÉ** : les mesures ci-dessus sont des crans et des dollys à La Réunion, pas les 45 s Atlantique → Mont-Blanc. **Aucun panoramique latéral** non plus.
- ⚠️ **LE TRAFIC OUVERT PAR LE GARDE DU ⑤ N'EST PAS MESURÉ.** Le quadtree se remet à raffiner en mode surface : c'est précisément le coût que la 1b bis disait ne pas avoir chiffré, et il est maintenant RÉEL au lieu d'être théorique. **C'est la première chose à mesurer avant d'ouvrir le drapeau `frontiereRendu`.**
- **LE DAMIER RESTE HORS DE LA LISTE**, comme avant : `block-grid.js` ajoute ses dalles voisines directement à `scene`, sans groupe ni interrupteur. Elles ne naissent que sous un tracé GPX qui déborde, et `entrerEnVol` les efface à chaque palier. **Le seuil ne change rien à leur sort — et l'orbite d'aujourd'hui a exactement le même angle mort.** Non reproduit avec un GPX.
- **Le bucket d'altitude a été injoignable par moments** (`ERR_CONNECTION_TIMED_OUT`, 404 préexistants) : une descente a dû être refaite deux fois, comme le §0 le prescrit.
...
- ✅ **TRANCHÉ PAR ADRIEN LE 2026-08-20 — CE QUE L'UTILISATEUR VOIT QUAND LE RÉSEAU NE SUIT PAS : UN INDICATEUR DISCRET.** Pas de voile, pas de message bloquant, pas de silence total : un signe non bloquant qui dit que le détail arrive. ⚠️ **Il remplace les 2,6 s de voile que `main.js:3413-3416` pose aujourd'hui alors que l'application est déjà libre** *(⚠️ le plan écrivait « 3408-3411 » ; vérifié le 2026-08-21, le `setTimeout(…, 2600)` est aux lignes 3413-3416 et `demBusy = false` à la ligne 3418, dans le `finally` qui s'exécute tout de suite — le repère a bougé, la thèse est exacte)*. À dessiner dans la Tâche 2, et à réutiliser par la Tâche 4 ter : **son ÉTAT est fabriqué et testé** (`etatIndicateur`, `src/monde/descente-bornee.js`), ~~**son DESSIN reste à faire**~~ ✅ **ET SON DESSIN EST FAIT LE 2026-08-21, TÂCHE 2** : `src/ui/indicateur-retard.js` + `.ce-retard` (`style.css`). Non bloquant, `role="status"`, **en niveaux jamais en pourcentage**, **éteint quand la mesure manque**. ⚠️ **MAIS IL N'EST PAS NOURRI PAR `etatIndicateur`, ET C'EST UNE MESURE QUI L'INTERDIT** — la même qui avait chassé `remplirBorne` du chemin du socle : sur un lien **oisif**, `debitObserve` rend 0,787 Mb/s et `zoomSoutenable` en tire z5 pour une demande de z12, donc l'indicateur resterait **allumé en permanence sur une connexion parfaite**. On lui passe un **fait** — `zoomEffectif`, la couverture réellement disponible sur l'emprise — et sa forme `{enRetard, niveaux}` est celle d'`etatIndicateur`, pour que la Tâche 1 puisse l'y brancher **sans adaptateur** le jour où R3 tiendra la caméra.
