...
| ~~GE2 bis~~ | **FUSIONNÉ** — C5 clic simple ×1,0000 (éteint élan/glissé/épingle) ; C4 double-clic ×2,000 vers le point cliqué à 0,0 px, droit ÷2,008 ; C6 cap **−50,000° × 8** — le second mode était une **violation antérieure de D16 ter** : pose de démarrage à 30,7–33,6 km autour de `SEUIL_NAISSANCE_M`, crop jamais né → retour au nadir jamais armé → inclinaison oblique du vol posée jusqu'à 2 400 km ; redressement automatique hors crop sauf `inclinaisonManuelle` ; C1 symétrie |ln| 0,002 (`Math.round(−0,5) = −0` perdait un demi-cran) ; C8 élan 10 %, mort en 1,4 s. Banc rouge 14 verts / 4 rouges — les 4 = **lectures du barème** (sens haut/bas vs Pro ; `rapportDistance` en unités de BLOC vs altitude ; D19 §2 sous `curseur`) → **à arbitrer par le noteur**. Note autoestimée 8,5 (9,5 / 7,5 selon l'unité). 4 799 · 0, audit 257 | `wt-ge2` | ✅ |
| ~~GE3 bis~~ | **FUSIONNÉ — la souris Google Earth notée 9,75/10** (tour 1 : 6). Les quatre lectures tranchées à la source : sens du clic droit = **bas/vers soi = zoom avant** (guide v4, test GE1 à l'envers) ; unité = **altitude `camGlobe`**, pas `d` de bloc (0,997 contre ×2,009 sur le même geste — `d` mesure un franchissement de palier) ; `GE_VISEE` ne bascule plus que le double-clic ; C6 = **inclinaison de départ jusqu'à 51,1° hors crop sur 33/33 chargements**, ramenée à 0° par le redressement (pas 1,476°/image). Seul manque : **roulis 3,7–3,9° au double-clic droit** (prix de la visée curseur) — quart de point. Reste à Adrien : ce roulis (seuil 2° ou visée centre pour le droit), `retoursNadir = 2`, la cause du vol de présentation oblique. 4 799 · 0, audit 257 | `wt-ge3` | ✅ |

**Aucun agent en vol. Toutes les campagnes du 2026-09-03/04 sont rentrées** : bathymétrie 9,33, BlueTopo 9,8 (arbitré), gestes Google Earth 9,75, carrés plats du sud, raffinement partiel, flou de zoom.
