// Export helpers — still-image capture at arbitrary resolution. The canvas uses
...
//
// ⚠️ 2026-09-04 — L'ENREGISTREUR MP4 DOUBLON EST PARTI D'ICI. `VideoExporter` et
// son orchestrateur `exportVideo` vivaient en bas de ce fichier et n'étaient
// appelés par PERSONNE : `exportVideo` avait une seule occurrence dans tout le
// dépôt, sa propre déclaration, et `VideoExporter` deux — sa déclaration et
// l'unique `new` fait par `exportVideo`. C'était un doublon historique :
// l'enregistreur VIVANT est `src/export-recorder.js`, qui importe `mediabunny`
// de son côté. La grappe partait donc entière — classe, orchestrateur, et
// l'import `mediabunny` de ce fichier, dont les cinq symboles (Output,
// Mp4OutputFormat, BufferTarget, CanvasSource, QUALITY_HIGH) ne servaient qu'à
// la classe. `mediabunny` RESTE dans package.json : export-recorder en vit.
// `saveState` / `applySize` / `restoreState`, partagés avec l'image fixe,
// restent eux aussi.
