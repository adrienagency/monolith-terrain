 ...
 */
export function plansFenetre(half, corner = 0) {
  const r = Math.max(0, Math.min(corner, half))
  // distance du centre à la diagonale tangente au cercle de coin
  const d = (half - r) * R2 + r
  const k = 1 / R2
  return [
 ...
 */
export function debordementCoin(corner) {
 ...
 */
export function dansFenetre(x, z, half, corner = 0) {
  for (const p of plansFenetre(half, corner)) {
